...
private:
	std::unique_ptr<SkinnedModel> mpModel;
...
void TestApp::Startup()
{
	...
	const uint32_t kNumInputLayouts = 5;
	...
	mpModel = modelLoader.LoadSkinnedModel("Models/Running.fbx");
	ASSERT(mpModel, "Failed to load model");
	ASSERT(mpModel->m_Header.meshCount > 0, "Model contains no meshes");

	float modelRadius = Length(mpModel->m_Header.boundingBox.max - mpModel->m_Header.boundingBox.min) * 0.5f;
	const Vector3 eye = (mpModel->m_Header.boundingBox.min + mpModel->m_Header.boundingBox.max) * 0.5f + Vector3(modelRadius * 0.5f, 0.0f, 0.0f);
